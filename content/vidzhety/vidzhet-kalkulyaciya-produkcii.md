---
description: >-
  Калькуляция продукта по умолчанию располагается только на странице продукта.
  Виджет «Калькуляция продукции» позволяет разместить калькуляцию продукции на
  любой странице сайта.
title: Виджет «Калькуляция продукции»
order: 1.8
---

## Общий вид

**Десктоп:**

![](<../../.gitbook/assets/image (851).png>)



**Мобильные устройства:**

![](<../../.gitbook/assets/image (824).png>)



:::info 

Калькуляция продукта по умолчанию располагается только на странице продукта.\
Виджет «Калькуляция продукции» позволяет превратить калькуляцию любого продукта в виджет и разместить его на любой странице сайта.

:::







## Как создать?

Чтобы создать виджет «Калькуляция продукции», в админ-панели сайта войдите в раздел «*Контент -> Виджеты»*, нажмите на кнопку «Добавить» в правом верхнем углу. В открывшемся окне найдите виджет «Калькуляция продукции» и нажмите «Создать».





## Параметры

### 

### Общие

Перед вами откроется форма с возможностью выбрать параметры виджета.

![](<../../.gitbook/assets/image (1562).png>)



Заполните поля и выберите параметры:

-  **Название** виджета\
   Внутреннее название для админ-панели. Нигде не отображается.

-  **Тип устройства**

   -  Универсальный -- виджет будет отображаться на всех устройствах;

   -  Для десктопа -- отображение будет только на компьютере/ноутбуке;

   -  Для мобильных устройств -- отображение только на мобильных устройствах.

-  **Продукция**\
   Выберите продукт, калькуляция которого будет отображаться в виджете.\
   В списке отображаются только включенные продукты.

-  **Отображение калькулятора**\
   Вариант отображения калькуляции в виджете.



:::tip 

Как один из вариантов применения: разместить данный виджет на посадочных страницах.

:::



:::note 

Не забудьте активировать виджет после создания. Это можно сделать в разделе «Контент -> Виджеты», путем переключения бегунка в состояние Вкл.

:::







## Порядок установки (2 вар.)

### 

### 1 вариант -- Через вставку кода

После сохранения всех параметров, скопируйте «Код для установки на сайт».

![](./vidzhet-kalkulyaciya-produkcii.png){width=888px height=188px}



Перейдите на нужную страницу или продукт, в режиме исходного кода вставьте код виджета в то место, которое необходимо.\
Готово!

![](./vidzhet-kalkulyaciya-produkcii-2.png){width=750px height=299px}



### 2 вариант -- Через редактор страниц

Перейдите в раздел "Контент -> Наполнение сайта -> Страницы" нажмите на название страницы. Вы окажитесь в редакторе страниц.\
Слева выберите необходимый виджет и вставьте в поле правее в нужном порядке.\
Готово!

![](./vidzhet-kalkulyaciya-produkcii-3.png){width=765px height=404px}