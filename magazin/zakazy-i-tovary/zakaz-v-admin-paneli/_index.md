...
**1\. Панель заказа без менеджера** (при отключенном [автоматическом распределении заказов](./../../../settings/polzovateli/nastroika-menedzherov#распределять-заказы-автоматически)).
...
-  [Счета на оплату](./../../bukhgalteriya/_index) (с печатью / без печати).

-  [УПД или Счет-фактура](./../../bukhgalteriya/_index) (с печатью / без печати / в формате XML).

-  [Технологическую карту](./dokumenty/tekhnologicheskaya-karta).

-  Этикетки на коробку, Этикетку и Маркировочную этикетку (доступно, если в заказе выбрана доставка транспортной компанией).
...
Если заказ оформлен новым клиентом (ранее не зарегистрированным), он попадает в раздел **Магазин -> Заказы ->** **вкладка «Неразобранные»**. Чтобы начать обработку, нажмите кнопку «Принять заказ». После этого клиент и заказ будут закреплены за текущим менеджером.
...
Доступна при подключенном модуле [«Отгрузки»](./../../otgruzki) и при условии, что заказ оплачен или для него установлена «Гарантия оплаты».
...
-  Клиент, Получатель, Адрес, Плательщик *(при смене этих полей система предлагает варианты, сохраненные в карточке клиента: **Магазин -> Контрагенты ->*** [***Клиенты***](./../../kontragenty#клиенты)*)*;
...
-  В общем списке заказов ([кнопка «Удалить»](./../vkladka-zakazy#удаление-заказа));
...
-  «Зачислить на [внутренний баланс](./../../../settings/integracii/sposoby-oplaty/_index#внутренний-баланс)».
...
-  Фамилия, Имя, Email и телефон клиента (данные из Корзины, пункт 2 «Заказчик», или из карточки клиента в разделе **Магазин -> Контрагенты ->** [**Клиенты**](./../../kontragenty#клиенты)).
...
Если в заказе был применен [промокод](./../../../marketing/skidki#промокод), он отобразится в этом блоке со ссылкой на настройки скидки в админ-панели.
...
3\. Кнопка [«Тех. карта»](./dokumenty/tekhnologicheskaya-karta)  ![](./zakaz-v-admin-paneli-22.png){width=70px height=41px} печать техкарты для конкретного товара
...
**Вариант «Сделать самим»:** Вы можете выставить дополнительный счет за корректировку макета. В списке появятся доп. услуги, отмеченные как «Спец.услуга» (**Справочник -> Свойства ->** [**Доп. услуги**](./../../../spravochnik/svoistva/dop-uslugi)).
