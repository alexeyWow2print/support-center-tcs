...
* **\<head> сайта**\
  **Что сюда размещать:** Код, который должен быть загружен в заголовке страницы. К примеру это мета-теги, канонические URL, код верификации сайта для поисковых систем (Яндекс.Вебмастер, Google Search Console) и некоторые виды трекеров.<br>
* **\<body> сайта**\
  **Что сюда размещать:** Скрипты и виджеты, которые должны отображаться или работать на всех страницах вашего сайта (но не в конструкторе товаров). Подходит для чат-ботов, виджетов обратного звонка, кнопок социальных сетей и т.п.<br>
* **\<body> конструктора**\
  **Что сюда размещать:** Код, который должен работать только внутри фоторедактора, редактора фотокалендарей или редактора полиграфии. Например скрипт с чат-ботом, или другим виджетом отображаемым в онлайн-редакторе.<br>
* **\<body> после контента**\
...
{% hint style="danger" %}
**Важно!** Коды сквозной аналитики **Яндекс.Метрики** и **Google Analytics** необходимо размещать в специальном разделе [Интеграции](../integracii/#analitika). Это гарантирует их корректную работу.
{% endhint %}
