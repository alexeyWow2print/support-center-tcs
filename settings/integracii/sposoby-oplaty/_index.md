...
-  *Компания-продавец* -- компания (настраивается в [Реквизитах](./../../oplata/rekvizity)), которая будет указываться в товарном чеке;
...
При выборе данного способа оплаты счет формируется автоматически и отправляется на email клиента. Счет формируется на основе данных, которые вы указали в разделе [Реквизиты](./../../oplata/rekvizity). После поступления средств необходимо перейти в [Счета клиентов](./../../../magazin/bukhgalteriya/scheta-klientov) (Админ-панель -> Магазин -> Бухгалтерия -> Счета клиентов). Рядом с нужным вам счетом нажмите "Оплатить" -- статус оплаты заказа сменится на "Оплачен".
